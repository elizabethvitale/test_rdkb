...
def configure_ip(mac, i, j):
    try:
        subprocess.call(["sudo", "ip", "link", "set", "dev", "eth0", "down"]) 
        subprocess.call(["sudo", "ip", "link", "set", "dev", "eth0", "address", mac])
        subprocess.call(["sudo", "ip", "link", "set", "dev", "eth0", "up"])
...
subprocess.call(["sudo", "ip", "link", "set", "eth0", "promisc", "on"])
wb = Workbook()
ws = wb.active
ws.title = "Test Data"
ws['A1'] = "Total IPv4"
